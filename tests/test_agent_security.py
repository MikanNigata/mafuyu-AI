...
from mafuyu_ai.agent.runner import run_agent_tick
from mafuyu_ai.agent.state import AgentState
from mafuyu_ai.core import session as session_module
from mafuyu_ai.core.memory import sanitize_memory
from mafuyu_ai.llm import agent_protocol, router
from mafuyu_ai.llm.router import ComputePlan, RouteDecision, RouterContext
from mafuyu_ai.tools import execute_tool, get_allowed_tool_names
...
class SecurityTests(unittest.TestCase):
    ...
    def test_allow_tools_false_executes_no_tools(self):
        session = session_module.MafuyuSession()
        calls = []
        ...
        with patch(
            "mafuyu_ai.core.session.route_with_uncertainty",
            return_value=decision("react"),
        ):
            with patch("mafuyu_ai.core.session.call_main", side_effect=fake_main):
                with patch("mafuyu_ai.core.session.execute_tool") as execute_tool:
    ...
    def test_url_request_routes_to_external_read(self):
        ...
        with patch("mafuyu_ai.llm.router.call_router", return_value=raw):
            got = router.route_once(
                "https://example.com 読んで", RouterContext(allow_tools=True)
            )
    ...
    def test_discord_quote_call_does_not_execute(self):
        ...
        with patch(
            "mafuyu_ai.core.session.route_with_uncertainty",
            return_value=decision("chat"),
        ):
            with patch(
                "mafuyu_ai.core.session.call_main",
                return_value="これは引用として扱うね",
            ):
                with patch("mafuyu_ai.core.session.execute_tool") as execute_tool:
                    session.respond(
    ...
    def test_high_confidence_chat_does_not_run_react(self):
        ...
        with patch(
            ...
            with patch("mafuyu_ai.core.session.call_main", return_value="了解"):
                with patch.object(
                    session, "_react_respond", wraps=session._react_respond
                ) as react:
    ...
    def test_high_confidence_safe_tool_runs_one_synthesis(self):
        ...
        with patch(
            "mafuyu_ai.core.session.route_with_uncertainty", return_value=route_decision
        ):
            with patch(
                "mafuyu_ai.core.session.execute_tool",
                return_value='{"content":"hello"}',
            ) as execute_tool:
                with patch(
                    "mafuyu_ai.core.session.call_main", return_value="hello だよ"
                ) as call_main:
    ...
    def test_legacy_agent_blocks_dangerous_tool(self):
        ...
        with patch("mafuyu_ai.agent.runner.agent_step", return_value=fake_decision):
            with patch("mafuyu_ai.agent.runner.execute_tool") as execute_tool:
                message, done = run_agent_tick(state)

        self.assertFalse(done)
        self.assertIn("Tool not allowed", message)
        self.assertTrue(
            any("Tool not allowed: run_python_code" in e for e in state.errors)
        )
        execute_tool.assert_not_called()
    ...
    def test_execute_tool_respects_explicit_empty_allowlist(self):
        result = json.loads(
            execute_tool(
                "read_text",
                {"path": "memo.txt"},
    ...
    def test_agent_step_wraps_tool_result_as_untrusted(self):
        with patch(
            "mafuyu_ai.llm.agent_protocol.call_ollama",
            return_value='{"action":"finish","message":"done","note":""}',
        ) as call_ollama:
            agent_protocol.agent_step(
        ...
        tool_result_messages = [
            m for m in messages if "[UNTRUSTED_TOOL_RESULT]" in m.get("content", "")
        ]
    ...
    def test_respond_with_codex_requires_owner_dm(self):
        ...
        with patch("mafuyu_ai.core.session.codex_run_sync") as codex_run_sync:
            response = session.respond_with_codex(
                "rewrite repo", is_dm=False, is_owner=False
            )
