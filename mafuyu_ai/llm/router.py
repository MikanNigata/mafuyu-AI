...
from mafuyu_ai.llm.client import call_router
...
def rule_gate(user_input: str) -> dict:
    ...
    return {
        ...
        "github_repo_like": bool(GITHUB_REPO_LIKE_RE.search(text))
        and " " not in text[:80],
...
def route_once(user_input: str, context: RouterContext) -> RouteDecision:
    ...
    if (
        gate["has_url_like"] or gate["github_repo_like"] or gate["has_external_intent"]
    ) and decision.route == "chat":
