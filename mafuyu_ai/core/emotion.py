...
from mafuyu_ai.settings import BASE_DIR
...
class EmotionSystem:
    ...
    def save(self):
        ...
        EMOTION_FILE.write_text(
            json.dumps(self.states, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    def update_state(
        self, user_id: str, affection_delta=0, mood_delta=0, energy_delta=0
    ):
        key = str(user_id)
    ...
    def get_prompt_text(self, user_id: str) -> str:
        ...
        if aff >= 90:
            aff_desc = "Love (Devoted)"
        elif aff >= 70:
            aff_desc = "High Trust (Close)"
        elif aff >= 40:
            aff_desc = "Neutral (Friend)"
        else:
            aff_desc = "Low (Stranger/Cold)"

        # Generate Mood description
        if mood >= 30:
            mood_desc = "Excellent (Happy/Playful)"
        elif mood >= 10:
            mood_desc = "Good (Positive)"
        elif mood >= -10:
            mood_desc = "Neutral (Calm)"
        elif mood >= -30:
            mood_desc = "Bad (Annoyed/Sarcastic)"
        else:
            mood_desc = "Terrible (Angry/Cold)"

        # Generate Energy description
        if energy >= 80:
            ene_desc = "High (Energetic)"
        elif energy >= 30:
            ene_desc = "Normal"
        else:
            ene_desc = "Low (Sleepy/Tired)"
