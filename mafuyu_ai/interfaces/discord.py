...
SessionKey = tuple[int | None, int | None]

sessions: dict[SessionKey, MafuyuSession] = {}
...
async def auto_talk_loop():
    ...
    if not last_channel_id or not last_user_id or not last_message_time:
        return
    ...
    session = get_session(user_id=last_user_id)
    loop = asyncio.get_running_loop()
...
async def on_message(message):
    global last_channel_id, last_user_id, last_user_name, last_message_time
...
async def clear_history(ctx):
    ...
    await ctx.reply(
        "履歴クリアしたよ！", allowed_mentions=discord.AllowedMentions.none()
    )
...
async def talk_to_mafuyu(ctx, *, message: str = None):
    if not message:
        await ctx.reply(
            "なに？話しかけてよ。", allowed_mentions=discord.AllowedMentions.none()
        )
        return
    ...
    async with ctx.channel.typing():
        ...
        response = await run_session_response(
            ...
            else user_has_allowed_role(ctx.author),
        )
...
def run() -> None:
    """Load configuration and run the Discord adapter."""
    ensure_runtime_dirs()
